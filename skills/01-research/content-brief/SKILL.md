---
name: content-brief
description: Compose a complete content brief — title, thesis, intent, audience, source ledger, editorial depth, schema hints, and EEAT plan — and persist via article.setBrief.
version: 0.1.0
...
3. **Title + thesis.** Compose a title that obeys the voice's tone slider and the topic's primary keyword. The title is concise enough to reduce truncation risk, specific, and free of clickbait fluff; do not fail it solely on character count. The thesis is the article's argument in one sentence — what the reader will know after reading. Capture both in `brief_json.title` and `brief_json.thesis`. Iterate internally if the first pass reads as generic; the brief is the contract that the rest of the pipeline executes against.
...
5. **Depth target.** Pull editorial depth from the cluster row's metadata when present; default to a medium-depth spoke, deep pillar, or short variant. The procedure can override via `args.target_word_count`, but treat that number as planning guidance only. Google has no preferred word count; never justify length as a ranking rule. Use the target to scope completeness, not to pad sections.
...
13. **Finish.** `run.finish` with `{article_id, sources_added, depth, editorial_depth, intent, source_quality_summary}`. Heartbeats fire after each source.add to keep the runs UI responsive.
...
- **DataForSEO/Firecrawl unavailable.** Best-effort fallback: cite from `serp-analyzer`'s prior run only. If no prior SERP audit exists, stop with `BLOCKED` and ask the procedure runner to run skill #2 or provide approved sources; do not call vendors outside the granted tool path.

## Variants

- **`light`** — one to five sources; smallest editorial scope; intended for short opinion or list articles.
- **`medium`** — six to eleven sources; default depth for the standard article shape.
