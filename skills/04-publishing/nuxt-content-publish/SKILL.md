...
inputs:
  ...
  target_id:
    ...
    type: int
    required: true
    description: The publish_targets.id whose kind='nuxt-content' the procedure controller selected for this run. Procedure 4 publishes to the primary target first, then queues secondary targets via publish_target.replicate.
outputs:
...
2. **Pre-flight integration and SEO preview.** When the target's `config_json.integration_credential_id` is set, call `integration.test` against that row. Then call `publish.preview(article_id, target_id)` and block if the rendered payload would introduce indexability, canonical, robots/noindex, public-image, structured-data, paid-link `rel`, or sitemap/lastmod regressions. Failing fast saves a partial commit.
...
6. **Compose the markdown body.** Take `edited_md` and rewrite every image reference from the daemon-side URL (`/generated-assets/<file>` or a future `/api/v1/assets/<id>` route) to the publish-side path (`/images/<asset-filename>`) where `asset-filename` is the file basename the skill copies in step 7. Other markdown content is unchanged: the compliance footer, the references block, the citation markers — all already rendered correctly by the draft / editor skills. Do NOT re-edit the body: any rewrite at publish time would invalidate the EEAT gate's audit on the same body.
7. **Copy assets into the target tree.** For each row in `asset.list`:
   - Resolve the source path. When the asset's `url` is an absolute external URL, download the bytes via `httpx.get` to a temp file. When the asset's `url` starts with `/generated-assets/`, read the file from `CONTENT_STACK_DATA_DIR/generated-assets/<file>` directly. When the asset's `url` is the daemon's own future `/api/v1/assets/<id>` route, read the file from the daemon-owned asset store. The daemon's filesystem layout is the canonical source; the URL is just the addressing scheme.
...
12. **Push.** Run `git -C <repo_path> push <git_remote> <branch>`. When the push returns non-zero (auth failure, remote rejected, network), capture the stderr in `runs.metadata_json.nuxt_content_publish.push_error`, leave the local commit in place for operator triage, and mark the publish as failed in step 14. Do not run destructive hard-reset rollback commands; a target repo may contain operator changes or debugging state that the skill must not destroy. Do not retry inside the skill — the procedure runner's retry policy handles transient failures; auth / 4xx errors should not be retried.
...
15. **Set canonical when this is the canonical target.** When the article's `canonical_target_id` is null AND this run is for the project's `is_primary=true` target, call `publish.setCanonical(article_id, target_id)` to make this target the canonical reference. When `canonical_target_id` is already set (a prior publish settled it), this step is a no-op.
...
- **Push failed (step 12).** Leave the local commit in place for operator triage, record the publish row with `status='failed'` and the error, surface in audit, and let the procedure agent decide whether to retry. Auth-class errors (401 / 403) should be surfaced to the operator, not retried. Do not run destructive rollback commands.
