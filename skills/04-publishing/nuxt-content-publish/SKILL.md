...
- **Push failed (step 12).** Roll back the local commit (`git reset --hard HEAD~1`), record the publish row with `status='failed'` and the error, surface in audit, and let the procedure agent decide whether to retry. Auth-class errors (401 / 403) should be surfaced to the operator, not retried.
