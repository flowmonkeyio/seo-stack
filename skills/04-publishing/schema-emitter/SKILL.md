...
Procedure 4 calls this skill after the alt-text-auditor (#14) and before the interlinker (#15). At this point the article is `eeat_passed`, the image package is in place (the auditor's flags are written), and the canonical URL may be provisional until the publish step returns a live URL. The schema-emitter composes a JSON-LD blob per applicable schema type, validates each blob, and writes one `schema_emits` row per type. The active publish skill consumes the rows and emits the JSON-LD inline in the rendered HTML / frontmatter; Procedure 4 defaults to `agent-publish` and swaps to `nuxt-content-publish` only when a primary active Nuxt target exists.
...
3. **Resolve the canonical URL.** When `article.canonical_target_id` is populated, the canonical URL is rendered from the target's `config_json.public_url_pattern` (e.g., `https://example.com/blog/{slug}`) interpolated with the article's `slug`. When `canonical_target_id` is null and a primary publish target exists, compute a provisional URL from that target. When no target exists, derive the best provisional URL from the project domain and slug, and mark `runs.metadata_json.schema_emitter.canonical_provisional=true`; the `agent-publish` step records the final live URL through `publish.recordExternal`. Persist whichever URL source was used in the per-type metadata so any audit can trace it.
...
- **Canonical URL resolution fails.** Means neither `articles.canonical_target_id`, a primary target, nor a project-domain fallback can produce a plausible URL. Without a canonical URL the `mainEntityOfPage` field cannot resolve; emit only schema types that can be valid without it and surface `runs.metadata_json.schema_emitter.canonical_unresolved=true`. Do not require a publish target just to satisfy schema.
