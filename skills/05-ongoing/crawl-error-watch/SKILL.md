...
description: Iterate published articles, call the granted GSC article-inspection/query tool per page, classify the verdict (PASS / PARTIAL / FAIL with reasons), and emit a topic with intent='technical-fix' for every FAIL or PARTIAL.
version: 0.1.0
...
- `project.get(project_id)` — returns `domain` (composes the `siteUrl` parameter for the URL Inspection call), `locale`, and `schedule_json` for tuning knobs (`gsc.url_inspection.run_cap` defaulting to 100 per run to leave headroom in the 2000/day quota).
...
- `gsc.inspectUrl` — hidden toolkit tool wrapping the URL Inspection API. Use `toolbox.describe` for the schema, then call it through `toolbox.call` with `site_url` and `inspection_url`. Returns the verdict object from Google; the skill normalises it to `{verdict: 'PASS' | 'PARTIAL' | 'FAIL', coverage_state, robots_txt_state, indexing_state, page_fetch_state, mobile_usability, rich_results, canonical_mismatch_reason?, last_crawl_time}`. The wrapper enforces the project's per-integration QPS budget (`default_qps=1.0` for GSC) and accumulates cost into `runs.metadata_json.cost.by_integration.gsc`.
...
4. **Inspect each URL.** For each article in the bounded roster, call `gsc.inspectUrl` through `toolbox.call` and let the daemon wrapper handle QPS pacing and quota accounting. Do not call ungranted bulk tools or native Google API paths. Capture the verdict object the wrapper returns. The verdict is a structured envelope with these fields:
...
5. **Classify FAIL versus PARTIAL.** Many `verdict` envelopes return PARTIAL for things the operator should still address. The skill applies a normalisation pass:
...
6. **Compose the topic per FAIL / PARTIAL.** For each article needing a topic:
