...
4. **Inspect each URL.** For each article in the bounded roster, call `gsc.queryArticle(article_id, since, until)` and let the daemon wrapper handle QPS pacing and quota accounting. Do not call an ungranted `gsc.bulk_inspect` or native Google API path. Capture the verdict object the wrapper returns. The verdict is a structured envelope with these fields:
