...
- `target.list(project_id)` — the publish target roster. The refresher reads `is_primary` and `kind` to decide whether the primary target is currently wired for procedure republish. Today only `kind='nuxt-content'` is wired end-to-end through the toolkit; WordPress/Ghost wrapper probes exist but their publish operations are deferred.
...
3. **Pre-flight publish path.** Call `target.list(project_id)` only to discover whether a primary active target exists. A wired `kind='nuxt-content'` target can use the concrete Nuxt publisher; otherwise the refresh re-publishes through `agent-publish`, which lets the main operator agent update the external system and record the targetless result. Do not abort solely because no target exists.
...
11. **Re-publish via the appropriate publish skill.** Delegate `nuxt-content-publish` only when the primary active target is `kind='nuxt-content'`. In every other case, delegate `agent-publish`: the main operator agent updates the existing external article using the remote id stored in `article_publishes.frontmatter_json` (when present) or the live URL, then records the new targetless publish row through `publish.recordExternal`. Capture the publish row id and published URL in the audit.
