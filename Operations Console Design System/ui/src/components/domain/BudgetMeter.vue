...
<template>
  <UiCard padding="md">
    <p class="text-2xs uppercase tracking-wider text-fg-subtle font-semibold">Spend{{ period ? ` · ${period}` : '' }}</p>
    ...
    <p class="text-xs text-fg-muted mt-1.5 tabular-nums">{{ pct().toFixed(0) }}% used</p>
  </UiCard>
</template>
