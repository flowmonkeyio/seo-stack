...
<template>
  <ol class="flex items-center gap-1 overflow-x-auto" aria-label="Article status">
    <li
      v-for="(s, i) in STEPS"
      :key="s"
    ...
    >
      <div
        class="flex items-center gap-1.5 px-2 h-6 rounded text-2xs font-medium uppercase tracking-wider"
        :class="{
      ...
      <div v-if="i < STEPS.length - 1" class="w-3 h-px bg-border-default"></div>
    </li>
  </ol>
</template>
