...
<template>
  ...
  >
    ...
    <div v-if="active && active.length" class="flex items-center flex-wrap gap-1.5">
      <UiBadge
        v-for="f in active"
        :key="f.key"
        tone="info"
      ...
      >
        ...
        <span>{{ f.value }}</span>
        <svg class="ml-1 -mr-0.5" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 6 6 18M6 6l12 12"/></svg>
      </UiBadge>
      <UiButton variant="link" size="sm" @click="$emit('clearAll')">Clear all</UiButton>
    </div>
  </div>
</template>
