    ...
    <template v-if="items">
      ...
      >
        <dt :class="[
          ...
          {{ item.label }}
          <p v-if="item.hint" class="text-2xs font-normal normal-case tracking-normal text-fg-disabled mt-0.5">{{ item.hint }}</p>
        </dt>
        <dd :class="[
          ...
          <slot :name="item.slot ?? item.label" :item="item">{{ dashIfNull(item.value) }}</slot>
        </dd>
      </div>
    </template>
    <slot v-else />
