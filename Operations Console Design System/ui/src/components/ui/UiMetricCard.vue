...
<template>
  ...
  >
    <p class="t-overline text-fg-subtle">{{ label }}</p>
    <div class="mt-1 flex items-baseline gap-2">
      <p v-if="loading" class="ui-skeleton block bg-bg-sunken animate-pulse rounded-xs" :style="{ width: '5ch', height: density === 'comfortable' ? '32px' : '24px' }" />
      <template v-else>
        <span :class="['font-semibold tabular-nums text-fg-strong', density === 'comfortable' ? 'text-2xl' : 'text-xl']">{{ value }}</span>
        <span v-if="unit" class="text-sm text-fg-muted">{{ unit }}</span>
      </template>
    </div>
    <div v-if="(delta !== undefined && delta !== null) || $slots.spark" class="mt-2 flex items-center justify-between gap-2">
      <span v-if="delta !== undefined && delta !== null" :class="['inline-flex items-center gap-1 text-xs font-medium', deltaToneClass]">
        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          <path :d="deltaIcon" />
        </svg>
        <span>{{ delta }}</span>
        <span v-if="deltaLabel" class="text-fg-subtle font-normal">· {{ deltaLabel }}</span>
      </span>
      <div v-if="$slots.spark" class="flex-1 max-w-[60%]">
        <slot name="spark" />
      </div>
    </div>
  </div>
