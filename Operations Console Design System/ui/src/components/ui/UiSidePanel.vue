...
const sideClass = computed(() => props.side === 'right' ? 'right-0' : 'left-0');
      ...
      >
        ...
        >
          <header class="ui-sidepanel__header flex items-start justify-between gap-3 px-5 py-4 border-b border-subtle shrink-0">
            <div class="min-w-0">
              <slot name="header">
                <h2 v-if="title" id="ui-sidepanel-title" class="t-h1 text-fg-strong">{{ title }}</h2>
                <p v-if="description" class="text-sm text-fg-muted mt-1">{{ description }}</p>
              </slot>
            </div>
            <UiIconButton v-if="!hideClose" aria-label="Close panel" size="sm" variant="ghost" @click="close">
          ...
          <footer v-if="$slots.footer" class="ui-sidepanel__footer flex items-center justify-end gap-2 px-5 py-3 border-t border-subtle bg-bg-surface-alt shrink-0">
