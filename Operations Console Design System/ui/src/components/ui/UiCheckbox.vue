  ...
  >
    <span class="relative inline-flex items-center justify-center mt-0.5">
      ...
      <span
        ...
        aria-hidden="true"
      >
        <svg v-if="modelValue && !indeterminate" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><path d="m5 12 5 5 9-12"/></svg>
        <svg v-else-if="indeterminate" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round"><path d="M5 12h14"/></svg>
      </span>
    </span>
    <span v-if="label || description || $slots.default" class="ui-checkbox__text flex flex-col">
      <span class="text-sm text-fg-default leading-tight">
        <slot>{{ label }}</slot>
      </span>
      <span v-if="description" class="text-xs text-fg-muted leading-tight mt-0.5">{{ description }}</span>
    </span>
  </label>
</template>
